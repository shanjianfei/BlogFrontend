<template>
  <div class="sub-comment-list-cell">
    ...
    <el-row class="sub-comment-bottom">
      ...
      <span>·</span>
      <span class="createtime">{{subComment.create_time | formatDate('yyyy年MM月dd日')}}</span>
      <el-link @click="giveLike(subComment.id)">赞</el-link>
      <span v-if="subComment.like" class="like-active">+{{subComment.like}}</span>
    </el-row>
  </div>
</template>
