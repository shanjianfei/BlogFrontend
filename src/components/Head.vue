<template>
  <div class="header-container"
              ...
              <el-menu :default-active="activeItem"
                ...
                <el-submenu :index="item.index"
                  ...
                  <el-submenu :index="category1.index"
                    ...
                    <el-menu-item :index="category2.index"
                                  v-for="(category2, index2) in category1.sub_categorylevel"
                                  v-if="category2.sub_categorylevel.length === 0"
                                  :key="index2">{{ category2.name }}</el-menu-item>
                    <el-submenu :index="category2.index"
                                v-for="(category2, index2) in category1.sub_categorylevel"
                                v-if="category2.sub_categorylevel.length > 0"
                                :key="index2">
                      <template slot="title">{{ category2.name }}</template>
                      <el-menu-item :index="category3.index"
                                    v-for="(category3,
                        index3) in category2.sub_categorylevel"
                                    :key="index3">{{ category3.name }}</el-menu-item>
                ...
                <el-menu-item :index="item.index"
                              v-for="(item, index) in category"
                              :key="index"
                              v-if="item.sub_category.length === 0">{{ item.name }}</el-menu-item>
    ...
    <transition name="el-zoom-in-left">
      <aside-nav v-if="asideNav"
                 @select="handleSelect"
                 @click.native="handleClickAsideNavWrap"
                 :articleCategory="articleCategory"></aside-nav>
    </transition>
  </div>
</template>
